...
if (isset ($_GET['pid']) && filter_var($_GET['pid'], FILTER_VALIDATE_INT, array('min_range' => 1))  ) { // Check for a product ID.
	$pid = $_GET['pid'];
	...
	if (isset($_SESSION['cart'][$pid])) {
		...
		// Display a message:
		echo '<p>Another unit of the product has been added to your shopping cart.</p>';
		
	} else { // New product to the cart.

		// Get the print's price from the database:
		require ('../mysqli_connect.php'); // Connect to the database.
		$q = "SELECT price FROM jm2292236_e_product WHERE product_id=$pid";
